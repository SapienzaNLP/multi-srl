...
class MultilingualSrlModel(pl.LightningModule):

    def __init__(self, hparams, padding_token_id=0, padding_target_id=-1):
        super(MultilingualSrlModel, self).__init__()
        ...
        self.role_scorer = nn.Linear(self.hparams.argument_encoding_size, self.num_roles)
    ...
    def training_step(self, batch, batch_index):
        ...
        predicate_identification_loss = MultilingualSrlModel._compute_classification_loss(
            scores['predicates'],
            labels['predicates'],
            2,
            ignore_index=self.padding_target_id,
        )
        sense_classification_loss = MultilingualSrlModel._compute_classification_loss(
            scores['senses'],
            labels['senses'],
            self.num_senses,
            ignore_index=self.padding_target_id,
        )
        argument_classification_loss = MultilingualSrlModel._compute_classification_loss(
            scores['roles'],
            labels['roles'],
            self.num_roles,
            ignore_index=self.padding_target_id,
    ...
    def validation_epoch_end(self, outputs):
        ...
        metrics = MultilingualSrlModel._compute_epoch_metrics(outputs)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--predicate_encoder_activation', type=str, default='identity')
        ...
        parser.add_argument('--sense_encoder_activation', type=str, default='identity')
        parser.add_argument('--sense_dropout_rate', type=float, default=0.5)
        ...
        parser.add_argument('--argument_lstm_num_layers', type=int, default=2)
        ...
        parser.add_argument('--argument_dropout_rate', type=float, default=0.5) 
        ...
        parser.add_argument('--language_model_min_learning_rate', type=float, default=1e-8)
