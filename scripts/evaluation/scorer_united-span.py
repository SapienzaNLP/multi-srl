        ...
        for line_no, line in enumerate(f):
            ...
            if not line:
                ...
                for token_id, frame_roles in zip(frames.keys(), roles):
                    try:
                        roles_dict[token_id] = convert_bio_to_spans(frame_roles)
...
def evaluate(gold_data:dict, pred_data:dict) -> dict:
    ...
    for united_srl_id, gold_sentence in gold_data.items():
        ...
        for token_id, gold_roles in gold_sentence['roles'].items():
            ...
            for gold_span, gold_role in gold_roles.items():
                if gold_role == 'V':
                    continue
                ...
                if gold_span in pred_roles:
                    pred_role = pred_roles[gold_span]
                    if pred_role != 'V' and pred_role not in role_f1:
            ...
            for pred_span, pred_role in pred_roles.items():
                if pred_role == 'V':
                    continue
                
                if pred_span not in gold_roles:
